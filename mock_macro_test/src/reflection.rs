...
pub fn test() {
    println!("Standalone");
    standalone();
    println!("outer {:?}", standalone.get_mock_id());

    println!("\nGeneric standalone");
    standalone_generic(0i32);
    standalone_generic(0u32);
    println!("outer {:?}", standalone_generic::<u32>.get_mock_id());
    ...
    println!("\nStruct impl");
    Struct::implem();
    println!("outer {:?}", Struct::implem.get_mock_id());

    println!("\nStruct generic impl");
    Struct::generic_impl(0i32);
    Struct::generic_impl(0u32);
    println!("outer {:?}", Struct::generic_impl::<u32>.get_mock_id());

    println!("\nGeneric struct impl");
    GenericStruct::<i32>::implem();
    GenericStruct::<u32>::implem();
    println!("outer {:?}", GenericStruct::<u32>::implem.get_mock_id());
    ...
    println!("outer {:?}", GenericStruct::<u32>::generic_impl::<u64>.get_mock_id());
...
fn standalone() {
    println!("inner {:?}", standalone.get_mock_id());
}

fn standalone_generic<T>(_: T) {
    println!("inner {:?}", standalone_generic::<T>.get_mock_id());
}
...
impl Struct {
    fn implem() {
        println!("inner {:?}", Self::implem.get_mock_id());
    }

    fn generic_impl<T>(_: T) {
        println!("inner {:?}", Self::generic_impl::<T>.get_mock_id());
    }
}
...
impl<T> GenericStruct<T> {
    ...
    fn generic_impl<U>(_: U) {
        println!("inner {:?}", Self::generic_impl::<U>.get_mock_id());
    }
}

trait Trait {
    fn func() {
        println!("inner {:?}", Self::func.get_mock_id());
    }

    fn generic_func<T>(_: T) {
        println!("inner {:?}", Self::generic_func::<T>.get_mock_id());
    }
}
...
trait ETrait<T, U> {
    fn e_fn<V, W>(&self, _: T, _: U, _: V, _: W) {
        println!("inner {:?}", Self::e_fn::<V, W>.get_mock_id());
    }
}
...
fn dude_what() -> ! {
    println!("inner {:?}", dude_what.get_mock_id());
    panic!();
}
...
pub fn test_e() {
    ...
    println!("outer {:?}", EStructDef::<&str, f64>::e_fn::<i64, u64>.get_mock_id());

    println!("\nEStructDef2");
    let e_struct_def_2 = EStructDef2(&*s, 1.2f64);
    e_struct_def_2.e_fn(1, &ss, 2i64, 3u64);
    println!("outer {:?}", EStructDef2::<&str, f64>::e_fn::<i64, u64>.get_mock_id());

    println!("\nEStruct");
    let e_struct = EStruct(&*s, 1.2f64);
    e_struct.e_fn(1, &ss, 2i64, 3u64);
    println!("outer {:?}", EStruct::<&str, f64>::e_fn::<i64, u64>.get_mock_id());

    println!("\nEStruct2");
    let e_struct2 = EStruct2(&*s, 1.2f64);
    e_struct2.e_fn(1, &ss, 2i64, 3u64);
    println!("outer {:?}", EStruct2::<&str, f64>::e_fn::<i64, u64>.get_mock_id());

    println!("\nEStruct2a");
    let e_struct2a = EStruct2(&*s, 1.2f32);
    e_struct2a.e_fn(1, &ss, 2i64, 3u64);
    println!("outer {:?}", EStruct2::<&str, f32>::e_fn::<i64, u64>.get_mock_id());

    println!("\nDude what");
    println!("outer {:?}", dude_what.get_mock_id());
//    dude_what();
}
